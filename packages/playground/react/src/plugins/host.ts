/**
 * My Plugin Host Script
 * 本地插件宿主脚本
 *
 * 演示如何使用 defineHostPlugin 创建宿主脚本（不打包模式）：
 * - RPC 方法供 View 层调用
 * - 向 View 层发送事件
 * - 访问宿主页面信息
 */

import type { MyPluginOptions } from './MyPlugin'
...
export default defineHostPlugin({
  ...
  rpc: {
    ...
    flashBackground() {
      // 使用闭包变量跟踪闪烁状态，防止重复触发
      const flashKey = '__devtools_flash_bg_my_plugin__'
      ...
      document.body.style.backgroundColor = '#4ade80' // 使用绿色区分
  ...
  setup(ctx) {
    ...
    addLog('🚀 My Plugin host script initialized (local mode)')
    ...
    console.log('[My Plugin] Host script loaded (local mode)')
    ...
    return () => {
      ...
      console.log('[My Plugin] Host script cleanup')
    }
  },
})
